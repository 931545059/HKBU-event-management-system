...
(function () {
    ...
    var Swiper = function (container, params, _scope, $compile) {
        ...
        s.callPlugins = function (name) {
            for (var i = 0; i < s._plugins.length; i++) {
                if (name in s._plugins[i]) {
                    s._plugins[i][name](arguments[1], arguments[2], arguments[3], arguments[4], arguments[5]);
                }
            }
        };
        ...
        s.emit = function (name) {
            // Trigger callbacks
            if (s.params[name]) {
                s.params[name](arguments[1], arguments[2], arguments[3], arguments[4], arguments[5]);
            }
            var i;
            // Trigger events
            if (s.emitterEventListeners[name]) {
                for (i = 0; i < s.emitterEventListeners[name].length; i++) {
                    s.emitterEventListeners[name][i](arguments[1], arguments[2], arguments[3], arguments[4], arguments[5]);
                }
            }
            // Trigger plugins
            if (s.callPlugins) s.callPlugins(name, arguments[1], arguments[2], arguments[3], arguments[4], arguments[5]);
        };
        s.on = function (name, handler) {
            name = normalizename(name);
            if (!s.emitterEventListeners[name]) s.emitterEventListeners[name] = [];
            s.emitterEventListeners[name].push(handler);
            return s;
        };
        s.off = function (name, handler) {
            var i;
            name = normalizename(name);
            if (typeof handler === 'undefined') {
                // Remove all handlers for such event
                s.emitterEventListeners[name] = [];
                return s;
            }
            if (!s.emitterEventListeners[name] || s.emitterEventListeners[name].length === 0) return;
            for (i = 0; i < s.emitterEventListeners[name].length; i++) {
                if(s.emitterEventListeners[name][i] === handler) s.emitterEventListeners[name].splice(i, 1);
            }
            return s;
        };
        s.once = function (name, handler) {
            name = normalizename(name);
    ...
    var Dom7 = (function () {
        ...
        Dom7.prototype = {
            ...
            //Events
            on: function (name, targetSelector, listener, capture) {
            ...
            off: function (name, targetSelector, listener, capture) {
                var events = name.split(' ');
            ...
            once: function (name, targetSelector, listener, capture) {
                var dom = this;
                if (typeof targetSelector === 'function') {
                ...
                function proxy(e) {
                    listener(e);
                    dom.off(name, targetSelector, proxy, capture);
                }
                dom.on(name, targetSelector, proxy, capture);
            },
            trigger: function (name, eventData) {
                for (var i = 0; i < this.length; i++) {
                    var evt;
                    try {
                        evt = new window.CustomEvent(name, {detail: eventData, bubbles: true, cancelable: true});
                    }
                    catch (e) {
                        evt = document.createEvent('Event');
